...
def process_video_cli(
    ...
    """Консольная функция цензурирования видеофайла."""
    if not os.path.exists(video_path):
        print(f"❌ Ошибка: Входной файл {video_path} не существует!")
        sys.exit(1)
    ...
    print(f"⚙️ Режим цензуры: {censor_mode} ({attenuation_db} dB)")
    print(f"🧠 Модель Whisper: {model_size} | Ускорение: {gpu_desc}")

    temp_wav = video_path + ".temp_input.wav"
    temp_out_wav = video_path + ".temp_censored.wav"
    processor = AudioProcessor()
    
    print("\n[1/5] 🎵 Извлечение аудиодорожки (FFmpeg)...")
    video_duration_sec = processor.extract_audio(video_path, temp_wav)
    if video_duration_sec <= 0:
        video_duration_sec = processor.get_media_duration(video_path)
    ...
    print("\n[3/5] 🔍 Поиск нецензурных выражений и генерация маркеров...")
    custom_bad = []
    whitelist = []
    ...
    print(f"📌 Маркеры для DaVinci сохранены: {csv_path}")
    ...
    print("\n[5/5] 🚀 Сборка итогового видеофайла (Stream Copy)...")
    processor.mux_video(video_path, temp_out_wav, output_path)
...
def print_stats_cli():
    ...
    print("==========================================")
    print(f"▶️ Обработано видеофайлов: {summary['total_videos']}")
...
def run_watch_cli(watch_folder: str, device: str, model_size: str, censor_mode: str):
    ...
    monitor = WatchFolderMonitor(watch_folder, on_new_video)
    monitor.start()
    try:
        import time
        while True:
...
def main():
    parser = argparse.ArgumentParser(
        description="AutoCens — Автоматическое запикивание/приглушение мата в видео с накопительной статистикой."
    )
    parser.add_argument("video", nargs="?", help="Путь к видеофайлу (.mp4, .mov, .mkv)")
    ...
    if args.gui or len(sys.argv) == 1 or not args.video:
        try:
            from gui.app import launch_gui
            launch_gui()
        except ImportError as e:
            print(f"Ошибка запуска GUI: {e}")
            print("Запуск с файлом через CLI: python main.py video.mp4")
        return
